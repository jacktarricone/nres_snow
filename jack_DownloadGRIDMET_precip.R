...
# download 1 WY of gridmet data (precip)
...
AOI <- aoi_get(state = states) # get shapefiles
...
# create water year variable for endDate and saving
wy <-year + 1

# 'prcp', 'tmin', 'tmax', 'palmer'
param <- 'prcp' 
start <- paste0(year,"-10-01") # shorter date period for testing code
end <- paste0(wy,"-09-30")

# read in 1 water year of precip data
system.time(
  climate_data <-getGridMET(AOI, param = param, startDate = start, endDate = end)
)

# convert from list of raster to SpatRaster
system.time(climate_data <-rast(climate_data$gridmet_prcp)) # change this for each of the four variables
climate_data
...
hdf_file_name <-paste0("precip_daily_wy_",wy,".h5")
...
h5createDataset(file = hdf_file_name, dataset = "precip",
...
system.time(h5write(climate_array, hdf_file_name,"precip"))

rm(climate_array) # remove looping through multiple years

}

system.time(gridmet_to_h5(year = year))
