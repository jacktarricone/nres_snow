...
# download 1 WY of gridmet data (tmax)
...
#        'energy_release', 'tmax', 'pet_alfalfa', 'pet_grass', 'vpd'
...
# read in 1 water year of tmax data
system.time(
  climate_data <-getGridMET(AOI, param = param, startDate = start, endDate = end)
)

# convert from list of raster to SpatRaster
system.time(climate_data <-rast(climate_data$gridmet_tmax)) # change this for each of the four variables
climate_data
...
setwd(file.path("/Volumes","jack_t","nres_project","gridmet_data","tmax")) 

# create file name with year
hdf_file_name <-paste0("tmax_daily_wy_",wy,".h5")
...
h5createDataset(file = hdf_file_name, dataset = "tmax",
...
system.time(h5write(climate_array, hdf_file_name,"tmax"))
